...
class Widget:
    ...
    def __init__(self, conversionService, widget):
        ...
        try:
            ...
            if self.panelType == 'graph':
                ...
                if 'fieldConfig' in widget and 'defaults' in widget['fieldConfig'] and 'max' in widget['fieldConfig'][
                    'defaults']:
                    limit = widget['fieldConfig']['defaults']['max']

                if 'options' in widget and 'fieldOptions' in widget['options'] and 'defaults' in widget['options'][
                    'fieldOptions'] and 'max' in widget['options']['fieldOptions']['defaults']:
            ...
            raise Exception('Error converting widget {}: {}'.format(self.id, err))
    ...
    def convertQueries(self, targets, range=True):
        ...
        if len(queries) == 0:
            print('No queries found for {}'.format(targets))
